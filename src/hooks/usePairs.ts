...
function mapPair(row: Record<string, unknown>): ContentPair {
  return {
    ...
    music: row.music as ContentItem | undefined,
    art: row.art as ContentItem | undefined,
    creator: row.creator as ContentPair['creator'],
  };
}
...
export function usePairsForContent(contentId: string | undefined, type: 'music' | 'art' | undefined) {
  ...
  const refetch = useCallback(async () => {
    ...
    if (error) {
      ...
      setPairs((data || []).map((r) => mapPair(r as Record<string, unknown>)));
    }
    setLoading(false);
  }, [contentId, type]);
